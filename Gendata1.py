...
def main():
    ...
    imgTrainingNumbers = cv2.imread("TrainingsBilder/datumInDots.png")            # read in training numbers image
    ...
    imgEdges = cv2.Canny(imgGray, 300, 400)

    cv2.imshow("Edges", imgEdges)
    binImEdges=cv2.inRange(imgGray,0,120)
    cv2.imshow("Binaer", binImEdges)

    newimgTresh = cv2.adaptiveThreshold(imgEdges,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,11,2)

    cv2.imshow("newTreshVonEdges", newimgTresh)

    #imgBlurred = cv2.GaussianBlur(imgGray, (5, 5), 0)                        # blur
    imgBlurred = cv2.GaussianBlur(imgEdges, (5, 5), 0)
    ...
    imgContours, npaContours, npaHierarchy = cv2.findContours(newimgTresh,        # input image, make sure to use a copy since the function will modify this image in the course of finding contours
                                                 cv2.RETR_EXTERNAL,                 # retrieve the outermost contours only
                                                 cv2.CHAIN_APPROX_SIMPLE)           # compress horizontal, vertical, and diagonal segments and leave only their end points
    #cv2.imshow("Copy", imgThreshCopy)
